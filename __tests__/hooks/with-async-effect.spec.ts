...
import { IAsyncEffectParams } from "../../src/types";

describe("With Async Effect hook", () => {
  test("should update and inject values from async function", () => {
    let status = "mount";
    let expectedResolver: any = null;

    const asyncAction = (): PromiseLike<any> | Promise<any> => ({
      then: (res?: any): any => {
    ...
    const params: IAsyncEffectParams<any> = {
      deps: [],
      asyncAction,
      disposeAction
    };
